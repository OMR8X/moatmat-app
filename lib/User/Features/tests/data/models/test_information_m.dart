...
class TestInformationModel extends TestInformation {
  ...
  factory TestInformationModel.fromJson(Map json) {
    ...
    return TestInformationModel(
      ...
      video: (json["videos"] as List).map((e) => VideoModel.fromJson(e,tests: true)).toList(),
      files: List.generate(
  ...
  toJson() {
    return {
      ...
      "price": price,
      "password": password,
      "videos": (videos?.isNotEmpty ?? false) ? video?.map((e) => VideoModel.fromClass(e).toJson(tests: true)).toList() : [],
      "images": images,
      "period": period,
      "files": files ?? <String>[],
